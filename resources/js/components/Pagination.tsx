...
// Bước 3 - Khi chuyển trang: component sẽ gọi Inertia.get(basePath, { ...filters, page })
...
export default function Pagination({ paginator, buildUrl }: Props) {
  ...
  const last = Number(paginator.last_page ?? paginator.meta?.last_page ?? paginator.last ?? 1);
  ...
  function goTo(page: number) {
    ...
    // Use Inertia.get with path + data so query params are constructed correctly
    Inertia.get(basePath, data, { preserveState: true, preserveScroll: true });
  }

  return (
    <nav className="flex items-center justify-between p-4" aria-label="Pagination">
      <div className="flex items-center space-x-2">
        <PaginationButton onClick={() => goTo(current - 1)} disabled={current <= 1}>
        ...
        {pages.map((p) => (
          <PaginationButton key={p} onClick={() => goTo(p)} active={p === current} aria-current={p === current ? 'page' : undefined}>
            {p}
          </PaginationButton>
        ))}
        ...
        <PaginationButton onClick={() => goTo(current + 1)} disabled={current >= last}>
